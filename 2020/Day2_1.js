...
/*  Part 1
/// Now that we have the data in an array, we can parse it...
var validCount = 0;
...
for (var i = 0, l = data.length; i < l; i++) {
    ...
    var rule = parsed[0];
    var bounds = rule.split('-');

    var lBound = Number(bounds[0]);
    var uBound = Number(bounds[1]);

    /// Determine count of letter in password.
    var count = (password.match(new RegExp(letter, 'g')) || []).length;

    if (count <= uBound && count >= lBound) {
        validCount++;
    }
}

console.log(`There are ${validCount} valid passwords in the set.`);
*/
...
for (var i=0, l=data.length; i < l; i++) {
    ...
    // Subtract 1 from each position so we get a proper index
    var pos1 = Number(bounds[0])-1;
    var pos2 = Number(bounds[1])-1;

    /// Determine if letter is in either of pos1 or pos2.
    var match1 = password.match(new RegExp(`^.{${pos1}}[${letter}]`));
    var match2 = password.match(new RegExp(`^.{${pos2}}[${letter}]`));
