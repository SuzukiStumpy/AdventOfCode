{
    var data = [
        ...
        "nk",
        "nuk",
        ""      // Extra blank line added to aid parsing [so we don't need a special case after the loop to process the final element
    ];

    /*  Part 1
    (function (){
        // Parse each row of the array.
        // If row is empty, then grab totals and reset running total.
        //   add group total to sum of counts.
        var sumOfCount = 0;
        var answers = {};

        for (var i=0; i < data.length; i++) {
            if (data[i] === '') {
                // Grab totals and add to running score and reset data pool
                sumOfCount += Object.keys(answers).length;
                answers = {};
            } else {
                // Parse out the total number of 'yeses'
                for (var j = 0; j < data[i].length; j++) {
                    answers[data[i].charAt(j)] = true;
                }
            }
        }

        console.log(`The sum of all counts is ${sumOfCount}`);
    })();
    */
    ...
    (function () {
        ...
        for (var i = 0; i < data.length; i++) {
            if (data[i] === '') {
                ...
                numberInGroup += 1;
                for (var j = 0; j < data[i].length; j++) {
                    var theChar = data[i].charAt(j);
                    if (!answers.hasOwnProperty(theChar)) {
                        answers[theChar] = 0;
                    }
                    answers[theChar] += 1;
                }
        ...
        console.log(`The sum of all questions answered by everyone is ${sumOfCount}`);
    })();
