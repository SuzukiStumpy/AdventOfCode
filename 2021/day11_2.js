...
{	
	...
	// For part 2 we execute indefinitely and break out at the appropriate point...
	while (true) {
		steps += 1;
		let flashCoords = [];
		...
		// Check to see if every single octopus has flashed.  If so, save
		// the step and exit the loop...
		let allFlashed = input.reduce( (p, c) => 
			p && c.reduce( (p1, c1) =>
				p1 && (c1.flashed === true)
			, true) 
		, true );
	...
	console.log(`The octopi have synchronized at step ${steps}.`);
}