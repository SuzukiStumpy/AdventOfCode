{
	...
	for (let i = 0, currentNum; currentNum = numbers[i]; i++) {
		
		// Check each board in sequence...
		for (let boardIdx = 0, thisBoard; thisBoard = boards[boardIdx]; boardIdx++) {
			// Mark the number.
			for (let rowIdx = 0, thisRow; thisRow = thisBoard[rowIdx]; rowIdx++) {
				...
				if (colIdx >= 0) {
					...
					if (winner > 0) {
						console.log( `We have a winner!!! Board: ${boardIdx}, Winning sum: ${winner}, Winning number: ${currentNum}. Solution: ${winner * currentNum}.`);
