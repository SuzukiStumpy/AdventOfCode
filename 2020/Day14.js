{
    ...
    // Note:  This test data causes an OOM crash in part 2 - only use for part 1 test
    /*   let program = [
    ...
    */
    ...
    function dec2bin(integer, l) {
        let b = (integer >>> 0).toString(2);
        if (b.length < l) {
            b = b.padStart(l, '0');
        }
        return b;
    ...
    function processMask(mask) {
        ...
        return [
            processMask(mask.replace('X', '0')),
            processMask(mask.replace('X', '1'))
        ].flat();
    };
    ...
    (function part1() {
        ...
        for (let i = 0; i < program.length; i++) {
            if (program[i].indexOf('mask') !== -1) {
                maskArray = program[i].match(/[X01]+$/gi)[0].split('');
            } else {
                let loc = program[i].match(/\[\d+\]/gi)[0].replace('[', '').replace(']', '');
                let value = dec2bin(parseInt(program[i].match(/\d+$/gi)), maskArray.length);

                // Apply the bitmask and store it
                value = value.split('').map((v, i) => (maskArray[i] === 'X') ? v : maskArray[i]).join('');
                mem[loc] = parseInt(value, 2);
            }
        }

        // Finally, tot up the resulting memory...
        let total = 0;
        for (let k in mem) {
            total += mem[k];
        }

        console.log(`Part 1:  Total in memory is: ${total}.`);
    })();
    ...
    (function part2() {
        ...
        for (let i = 0; i < program.length; i++) {
            if (program[i].indexOf('mask') !== -1) {
                srcMask = program[i].match(/[X01]+$/gi)[0];

            } else {
                let loc = dec2bin(program[i].match(/\[\d+\]/gi)[0].replace('[', '').replace(']', ''), srcMask.length);
                let value = parseInt(program[i].match(/\d+$/gi)[0]);

                // Apply the mask to the location...
                loc = loc.split('');
                for (let j=0; j<loc.length; j++){
                    if (srcMask[j] === 'X') {
                        loc[j] = 'X';
                    } else {
                        loc[j] = srcMask[j] | loc[j];
                    }
                }
                loc = loc.join('');
                maskArray = processMask(loc);

                for (let j = 0, mask; mask = maskArray[j]; j++) {
                    mem[parseInt(mask, 2)] = parseInt(value);
                }
            }
        }
        ...
        console.log(`Part 2:  Total in memory is: ${total}.`);
    })();
}