{
    class Coordinate {
        ...
        toString() { return `${Coordinate.type}(${this.x},${this.y},${this.z})` };
        get() { return { 'x': this.x, 'y': this.y, 'z': this.z } };
    }

    class ActiveCells {
        #active = [];
        #bounds = { lower: new Coordinate(0, 0, 0), upper: new Coordinate(0, 0, 0) };
        constructor(i) {
        ...
        add(x, y, z) {
            this.#active.push(new Coordinate(x, y, z));
        ...
        isActive(x, y, z) { return this.#active.map(v => v.toString()).indexOf(new Coordinate(x, y, z).toString()) !== -1 };
        countActiveNeighbours(x, y, z) {
            return this.#active.reduce((p, c) =>
                (c.x >= x - 1 && c.x <= x + 1 &&
                    c.y >= y - 1 && c.y <= y + 1 &&
                    c.z >= z - 1 && c.z <= z + 1 &&
                    !(c.x === x && c.y === y && c.z === z)) ? p + 1 : p, 0);
        }
    }
        ...
        for (let x = active.bounds.lower.x - 1; x <= active.bounds.upper.x + 1; x++) {
            for (let y = active.bounds.lower.y - 1; y <= active.bounds.upper.y + 1; y++) {
                for (let z = active.bounds.lower.z - 1; z <= active.bounds.upper.z + 1; z++) {
                    let a = active.isActive(x, y, z);
                    let n = active.countActiveNeighbours(x, y, z);
