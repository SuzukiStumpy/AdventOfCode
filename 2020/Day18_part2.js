{
    ...
    function evaluate(expression) {
        ...
        expression = expression.replaceAll(' ', '');  // Remove all the whitespace

        while (expression.match(/^\d+$/g) === null) {
            idx = expression.indexOf('(');

            if (idx !== -1) {  // Factor out brackets
                // Find the matching closing bracket
                let bcount = 1;
                for (idx2 = idx + 1; idx2 < expression.length; idx2++) {
                    if (expression.charAt(idx2) === '(') {
                        bcount += 1
                    } else if (expression.charAt(idx2) === ')') {
                        bcount -= 1;
                        if (bcount === 0) break;
                    }
                }
                let subExpr = expression.substring(idx, idx2 + 1);
                expression = expression.replace(subExpr, evaluate(subExpr.substring(1, subExpr.length - 1)).toString());
            } else {
                // Now, process addition
                while (expression.indexOf('+') !== -1) {
                    let subExpr = expression.match(/(\d+)\+(\d+)/);
                    let e = ops['+'](subExpr[1], subExpr[2]).toString();
                    expression = expression.replace(subExpr[0], e);
                }

                while (expression.indexOf('*') !== -1) {
                    // Followed by the rest of the expression...
                    let subExpr = expression.match(/^(\d+)([+*]{1})(\d+)/);
                    let e = ops[subExpr[2]](subExpr[1], subExpr[3]).toString();
                    expression = expression.replace(subExpr[0], e);
                }
            }
        }
    ...
    for (let i = 0; i < data.length; i++) {
        let v = evaluate(data[i]);
        console.log(`${data[i]} == ${v}`);

        total += v;
    }

    console.log(`Part 2:  The sum of all values is: ${total}`);
}