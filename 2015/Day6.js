{
    ...
    let lights = Array(1000).fill(Array(1000).fill(false));

    function parse(i) {
        let p = i.match(/(.*) (\d+),(\d+) \w+ (\d+),(\d+)/);

        return {
            'command': p[1],
            'lbounds': {
                'x': Math.min(parseInt(p[2]), parseInt(p[4])),
                'y': Math.min(parseInt(p[3]), parseInt(p[5])),
            },
            'ubounds': {
                'x': Math.max(parseInt(p[2]), parseInt(p[4])),
                'y': Math.max(parseInt(p[3]), parseInt(p[5])),
            }
        }
    }
debugger
    for (let i = 0; i < data.length; i++) {
        let inst = parse(data[i]);

        for (let x = inst.lbounds.x; x <= inst.ubounds.x; x++) {
            for (let y = inst.lbounds.y; y <= inst.ubounds.y; y++) {
                switch (inst.command) {
    ...
    // now use reduce to get the number of 'on' bulbs
    let lit = lights.reduce((p, v) => p + v.filter( v => {return v} ).length, 0);
    console.log(`Part 1:  There are ${lit} lit bulbs`);
}