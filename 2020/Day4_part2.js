{
    ...
    var data = String.raw `eyr:2029 pid:157374862
...
hgt:167cm eyr:2026`;
    ...
    for (var i=0, passport; passport = dataArray[i]; i++) {
        passport = passport.replace(/\n/gi, ' ');
        var regex = /\b(?:byr|iyr|eyr|hgt|hcl|ecl|pid|cid)\b/gi;
        var matches = passport.match(regex);
        var isValid = false;

        // If all 8 entries are present, then we're good.
        if (matches.length === 8) {
            isValid = true;
        // If only 7 entries are present, check to see if the missing one is 'cid'
        } else if (matches.length === 7) {
            var rematch = matches.join(' ');
            if (rematch.indexOf('cid') === -1) {
                isValid = true;
            }
        }

        // Now we need to parse out the validity according to the rulebase...
        if (isValid === true) {
            var passportFields = passport.split(' '); // Get each of the entries in it's own element

            // Parse each field
            for (var j = 0, fieldData; field = passportFields[j]; j++) {
                field = field.split(':');
                switch (field[0]) {
                    case 'byr':  // Four digits; 1920 <= value <= 2002
                        var value = Number(field[1]);
                        isValid = (value <= 2002 && value >= 1920) ? true : false;
                        break;

                    case 'iyr':  // Four digits; 2010 <= value <= 2020
                        var value = Number(field[1]);
                        isValid = (value <= 2020 && value >= 2010) ? true : false;
                        break;

                    case 'eyr':  // Four digits; 2020 <= value <= 2030
                        var value = Number(field[1]);
                        isValid = (value <= 2030 && value >= 2020) ? true : false;
                        break;

                    case 'hgt':  // Number followed by either cm or in; If cm 150 <= value <= 193; If in 59 <= value <= 76
                        var value = parseInt(field[1]);
                        
                        if (field[1].match(/cm/gi)) {
                            isValid = (value <= 193 && value >= 150) ? true : false;
                        } else if (field[1].match(/in/gi)) {
                            isValid = (value <= 76 && value >= 59) ? true : false;
                        } else {
                            isValid = false;
                        }
                        break;

                    case 'hcl':  // # followed by exactly 6 hexadecimal digits
                        isValid = /^#[0-9a-f]{6}$/gi.test(field[1]);
                        break;

                    case 'ecl':  // one of: amb, blu, brn, gry, grn, hzl, oth
                        isValid = /^(amb|blu|brn|gry|grn|hzl|oth)$/gi.test(field[1]);
                        break;

                    case 'pid':  // Nine digits including leading zeroes
                        isValid = /^[0-9]{9}$/gi.test(field[1]);
                        break;
                ...
                // Break out if we hit an invalid field.
                if (isValid === false) {
                    break;
                }
            }
