{
    ...
    // Parses out the grammar defined in the rules array.
    function expand(input, idx) {
        if (input.match(/^"\w*"$/gi)) {
            return input.replaceAll('"', '');
        }

        if (input.match(/^\d+$/gi)) {
            return expand(rules[parseInt(input), parseInt(input)]);
        }

        // TODO: Somehow need to handle recursion in here for part 2
        let ia = input.split('|');
        ia = ia.map(v => {
            let s = v.match(/[^\s]+/gi);
            let r = '(';
            s.forEach(v => {
                r += expand(v, parseInt(v));
            });
            r += ')';
            return r;
        });
...
debugger
    // Get the rules into a proper order...
    input.forEach((v, i) => {
        let idx = parseInt(v.substring(0, v.indexOf(':')));
        rules[idx] = v.substring(v.indexOf(':') + 1).trim();
    });

    // Parses out the grammar and somewhat simplifies the returned string.
    let grammar = expand(rules[0], 0).replace(/\(\w+\)/gi, m => m.substring(1, m.length - 1));
    // Example string returned:
    //  ((a(((aa|bb)(ab|ba))|((ab|ba)(aa|bb)))b))

    let re = new RegExp(`^${grammar}$`, 'i');
    let c = 0;
    messages.forEach((v) => v.match(re) ? c += 1 : null);

    console.log(`Part 1:  There are ${c} matching strings.`);

    // Part 2:  Fix rules 8 and 11
    rules[8] = '42 | 42 8';
