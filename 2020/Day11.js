...

{
    ...
    */
    ...
    // const MAX_OCCUPANCY = 4;  // For part 1
    const MAX_OCCUPANCY = 5;   // For part 2
    const OCCUPIED = '#';
    const VACANT = 'L';
    const FLOOR = '.';
    ...
    // For part 1
    /*
    function getOccupationCount(state, row, col) {
        let count = 0;

        for (let checkRow = row - 1; checkRow <= row + 1; checkRow++) {
            for (let checkCol = col - 1; checkCol <= col + 1; checkCol++) {
                if (checkRow >= 0 && checkRow < state.length && checkCol >= 0 && checkCol < state[checkRow].length) {
                    if (!(checkRow === row && checkCol === col) && state[checkRow].charAt(checkCol) === OCCUPIED) {
                        count += 1;
                    }
                }
            }
    ...
    */
    ...
    function getOccupationCount(state, row, col) {
        ...
        for (let delta in directions) {
            let currentRow = row;
            let currentCol = col;

            while(true) {
                currentRow += directions[delta][0];
                currentCol += directions[delta][1];

                if (currentRow >= 0 && currentRow < state.length && currentCol >= 0 && currentCol < state[0].length) {
                    let seatState = state[currentRow].charAt(currentCol);
                    if (seatState !== FLOOR) {
                        count += (seatState === OCCUPIED) ? 1 : 0;
                        break;
                    }
                } else {
                    break;
...
//debugger
            ...
            for (let row = 0; row < data.length; row++) {
                for (let col = 0; col < data[0].length; col++) {
                    let cell = startState[row].charAt(col);

                    if (cell === VACANT) {
                        // Check that there are no occupied seats around it
                        if (getOccupationCount(startState, row, col) === 0) {
                            data[row] = data[row].replaceAt(col, OCCUPIED);
                            seatsChanged += 1;
                        }
                    } else if (cell === OCCUPIED) {
                        // Check if four or more seats are occupied
                        if (getOccupationCount(startState, row, col) >= MAX_OCCUPANCY) {
                            data[row] = data[row].replaceAt(col, VACANT);
                            seatsChanged += 1;
                        }
                    }
                }
            ...
            if (seatsChanged > 0) {
                console.log(`After ${counter} iterations, ${seatsChanged} seats changed state:`);
                for (let row = 0; row < data.length; row++) {
        ...
        console.log(`Stable state reached after ${counter - 1} iterations`);
        var occupiedCount = 0;
        for (let row = 0; row < data.length; row++) {
            for (let col = 0; col < data[row].length; col++) {
                if (data[row].charAt(col) === OCCUPIED) {
                    occupiedCount += 1;
                }
            }
        }
